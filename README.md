...
运行后，将光标放在任意输入框中，**长按 Ctrl+F2 键**开始语音输入。
...
2. **开始输入**：长按 Ctrl+F2 键，看到 `(( 🎤 ))` 提示后开始说话
3. **结束输入**：松开 Ctrl+F2 键，文字将自动输入到光标位置
4. **退出程序**：按 ESC 键退出

### 快捷键配置

可以在 `config.yaml` 中自定义快捷键：

```yaml
app:
  hotkey:
    key_combination: "ctrl+f2"  # 录音快捷键
    exit_key: "esc"              # 退出按键
```

支持的快捷键格式：`ctrl+a`、`alt+f1`、`shift+f2` 等组合键。
...
- **config.yaml** (基础版) - 包含最常用的配置项，适合大多数用户
- **config.advanced.yaml** (高级版) - 包含所有参数，供需要深度定制的用户参考

### 基础配置

编辑 `config.yaml` 可以修改以下常用参数：

```yaml
app:
  mode: "stream"  # "stream" (流式) 或 "offline" (离线)
  hotkey:
    key_combination: "ctrl+f2"  # 录音快捷键
    exit_key: "ctrl+esc"        # 退出键

asr:
  model_type: "sense_voice"     # "paraformer" 或 "sense_voice"
```

### 高级配置

如需调整线程数、VAD 阈值等高级参数，可以将 `config.advanced.yaml` 中的相应部分复制到 `config.yaml` 中修改。
...
VAD（语音活动检测）默认配置已内置到代码中：

- **类型**: `ten_vad`（推荐）或 `silero_vad`
- **阈值**: 0.5
- **最短静音时间**: 0.1 秒
- **最短语音时间**: 0.25 秒
- **最长语音时间**: 5.0 秒
...
如需调整这些参数，请参考 `config.advanced.yaml` 中的 `vad` 配置部分。
...
1. 检查是否有其他程序占用 Ctrl+F2 键
2. 确认程序有足够的系统权限
3. 在 Linux 上可能需要 root 权限
4. 可以在 `config.yaml` 中修改快捷键为其他组合键
